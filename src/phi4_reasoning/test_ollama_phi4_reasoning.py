#!/usr/bin/env python3
"""
Script de benchmark individual para el modelo Microsoft Phi-4 Reasoning
Modelo de razonamiento avanzado con capacidades mejoradas
"""
...
def summarize(stats, elapsed, system_metrics_summary=None):
    """
    Combina métricas de Ollama con métricas del sistema
    """
...
def main():
    ap = argparse.ArgumentParser(description="Benchmark de Microsoft Phi-4 Reasoning")
    ap.add_argument("--host", default="http://localhost:11434", help="URL del servidor Ollama")
    ap.add_argument("--model", default="phi4-reasoning:latest", help="Modelo a usar (default: phi4-reasoning:latest)")
    ap.add_argument("--prompt", default="Explain step by step how to solve the equation: 2x + 5 = 15. Show your reasoning process.",
                    help="Prompt a evaluar (por defecto: problema de razonamiento matemático)")
    ...
    ap.add_argument("--ctx", type=int, default=4096, help="num_ctx (ventana de contexto, default: 4096)")
    ap.add_argument("--temp", type=float, default=0.0, help="temperature (default: 0.0 para respuestas determinísticas)")
    ...
    metrics = []
    print(f"\n=== Iniciando {args.runs} ejecuciones de benchmark para Phi-4 Reasoning ===")
    ...
    for i in range(args.runs):
        ...
        # Mostrar métricas básicas (como antes)
