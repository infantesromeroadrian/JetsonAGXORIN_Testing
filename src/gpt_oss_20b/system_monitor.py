#!/usr/bin/env python3
"""
Sistema de monitoreo de recursos del sistema para benchmarks de GPT-OSS 20B
Captura métricas de CPU, GPU, RAM, temperatura durante las ejecuciones del modelo de 20B parámetros
"""
...
class SystemMonitor:
    """Monitor continuo de recursos del sistema para GPT-OSS 20B"""
...
# Funciones de conveniencia para usar con los scripts de GPT-OSS 20B
...
if __name__ == "__main__":
    # Ejemplo de uso del monitor para GPT-OSS 20B
    print("=== Test del Sistema de Monitoreo - GPT-OSS 20B ===")
    ...
    print(f"\nMedición instantánea:")
    print(f"CPU: {instant.cpu_percent:.1f}% @ {instant.cpu_freq_current:.0f}MHz")
    print(f"RAM: {instant.ram_used_gb:.1f}/{instant.ram_total_gb:.1f}GB ({instant.ram_percent:.1f}%)")
    
    if instant.gpu_usage_percent is not None:
        print(f"GPU: {instant.gpu_usage_percent:.1f}%")
    if instant.cpu_temp is not None:
        print(f"CPU Temp: {instant.cpu_temp:.1f}°C")
    if instant.gpu_temp is not None:
        print(f"GPU Temp: {instant.gpu_temp:.1f}°C")
    if instant.power_consumption_watts is not None:
        print(f"Power: {instant.power_consumption_watts:.1f}W")
    ...
    print(f"Duración: {summary['monitoring_duration_s']:.1f}s")
    print(f"Muestras: {summary['total_samples']}")
    ...
    ram_stats = summary['ram_usage_percent'] 
    if ram_stats['count'] > 0:
        print(f"RAM: {ram_stats['mean']:.1f}% (min={ram_stats['min']:.1f}%, max={ram_stats['max']:.1f}%)")
